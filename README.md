...
每天有 **5 个触发时点**，但其中 4 次是"今日已领"的空跑。如果每次都推，一天 5 条微信，你很快就会把它关掉 —— 那就等于没配。所以脚本只在两种情况下推送：

| 场景 | 是否推送 | 标题 |
|---|---|---|
| 领取成功 | ✅ 推 | `WorkBuddy 签到成功 +100 积分` |
| 出错（令牌失效 / 网络异常 / 令牌缺失） | ✅ 推 | `WorkBuddy 签到异常，需要处理` |
| 今日已签到（skip） | ❌ 静默 | — |

正常情况：**每天最多 1 条**（首次领取成功时）。正文示例：
